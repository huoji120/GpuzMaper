...
uint64_t gpuz::TranslateVirtualAddress(uint64_t directoryTableBase, LPVOID virtualAddress)
{
...
BOOLEAN gpuz::ReadPhysicalAddress(uint64_t Address, PVOID buffer, SIZE_T Length)
{
	...
	{
		//printf("address:0x%016I64x  value:0x%016I64x\n", in.dwAddress, *(ULONG64*)(value));
		for (int i = 0; i < in.dwLength; i++)
		{
			*((char*)buffer + i) = *((char*)value + i);
		}
		//memcpy(buffer, *(PVOID*)value, Length);
	}
