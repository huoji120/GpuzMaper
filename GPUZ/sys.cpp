...
/* Wrapper Routine For Loading Driver */
BOOLEAN sys::LoadDriverRoutine(std::string ServiceRegKey)
{
	auto& utils = Utils::instance();
	if (!DisableDSE())			// Disable DSE For Loading Unsigned Drivers
	{
		Driver->~gpuz();
	}
	else if (!NT_SUCCESS(utils.LoadDriver(ServiceRegKey)))		// If Loading Driver Fails, re enable DSE.
	{
		EnableDSE();											// Re Enable DSE.
		Driver->~gpuz();
	}
	else if (!EnableDSE())										// Re Enable DSE.
	{
		Driver->~gpuz();
	}
	else
	{
		...
		std::cin.get();
		if (!NT_SUCCESS(utils.UnloadDriver(ServiceRegKey)))		// Unload The Driver When Finished
		{
			Driver->~gpuz();
			return FALSE;
		}
